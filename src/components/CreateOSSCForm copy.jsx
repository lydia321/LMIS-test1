...
const GET_ZONES = gql`
  query ZONE($regionId: ID!) {
    base_zone(regionId: $regionId) {
      id
      name
    }
  }
...
const CreateOSSCForm = ({ isOpen, onClose }) => {
  ...
  // Fetch Woreda based on selected zone
  const { data: woredasData, loading: loadingWoredas, error: woredasError } = useQuery(GET_WOREDA, {
    variables: {
      where: { zone_id: { _eq: selectedZoneId } }, // Assuming zone_id is the field to filter Woreda
    },
    skip: !selectedZoneId,
  });
